...
def myparse(responce_source):
    ...
    for item in items:
        ...
        stars = item.xpath(XPATH_STARS)[0].replace(' su 5 stelle', '')
        prime = item.xpath(XPATH_PRIME)[0] == 'Amazon Prime'
        link = "https://www.amazon.it" + item.xpath(XPATH_LINK)[0]
        price = item.xpath(XPATH_PRICE)[0]
